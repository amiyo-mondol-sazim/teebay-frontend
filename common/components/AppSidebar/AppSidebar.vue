...
<template>
  <UiSidebar default-open class="flex flex-col justify-between border-r">
    ...
    <UiSidebarContent>
      <span style="background: linear-gradient(to right, white, rgb(223, 225, 226), white)" class="my-5 block h-0.25" />
      <nav>
        <ul class="flex flex-col gap-2 px-4">
          <li v-for="navItem in navItems" :key="navItem.name">
            <UiButton
              ...
              "
            >
              <Icon :name="navItem.icon" :class="cn('size-7', isActive(navItem.path) ? 'text-primary' : '')" />
              {{ navItem.name }}
            </UiButton>
          </li>
        </ul>
    ...
    <UiSidebarFooter class="px-4 pb-6">
      <div v-if="user" class="border-gray-30 flex items-center gap-2 rounded-full border p-1.5">
        <div
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="truncate text-sm text-gray-200 italic">{{ user?.email ?? 'n/a' }}</div>
        </div>
        ...
        <ConfirmationModal
          ...
          @action="logout"
        >
          <UiButton variant="ghost" size="lg" class="text-gray-60 hover:text-gray-80 size-12 rounded-full">
            <Icon name="ph:sign-out" class="size-5" />
          </UiButton>

          <template #action>
            <UiButton variant="primary" class="dark-gradient-shadow h-10.5 px-5 py-3 text-base font-semibold">
              Sign Out
            </UiButton>
          </template>
        </ConfirmationModal>
