...
export const useCategorySelector = (categories: Ref<string[]>) => {
  ...
  const isValidCategory = (category: string): category is typeof AVAILABLE_CATEGORIES[number] => {
    return AVAILABLE_CATEGORIES.includes(category as typeof AVAILABLE_CATEGORIES[number]);
  };
  ...
  const isAtMaxLimit = computed(() => categories.value.length >= MAX_CATEGORIES);
